...
define("NONE", "/^(CREATEFRAME|PUSHFRAME|POPFRAME|RETURN|BREAK|ADDS|SUBS|MULS|IDIVS|DIVS|LTS|GTS|EQS|ANDS|ORS|NOTS|INT2CHARS|STRI2INTS|CLEARS|INT2FLOATS|FLOAT2INTS)$/i");
define("NONE_COMMENT", "/^(CREATEFRAME|PUSHFRAME|POPFRAME|RETURN|BREAK)#.*$/i");
define("RET", "/^RETURN$/i");
define("SVAR", "/^(DEFVAR|POPS)$/i");
define("LBL", "/^(LABEL)$/i");
define("JUMP", "/^(CALL|JUMP|JUMPIFEQS|JUMPIFNEQS)$/i");
define("SYMB", "/^(DPRINT|EXIT|WRITE|PUSHS)$/i");
define("VAR_SYMB", "/^(MOVE|INT2CHAR|STRLEN|TYPE|NOT|INT2FLOAT|FLOAT2INT)$/i");
define("VAR_TYPE", "/^READ$/i");
define("VAR_SYMB_SYMB", "/^(ADD|SUB|MUL|IDIV|STRI2INT|CONCAT|GETCHAR|SETCHAR|LT|GT|EQ|AND|OR|DIV)$/i");
...
define("TYPE", "/^(int|bool|string|float)$/");
...
function parse_symbol($symbol)
{
    ...
    else if (preg_match(FLT, $symbol))
    {
        // the symbol is a float
        if (strlen(substr($symbol, 6)) == 0)
        {
            exit(LEX_SYN_ERR);
        }
        $xml_file->add_intsruction_argument(substr($symbol, 6), "float");
    }
